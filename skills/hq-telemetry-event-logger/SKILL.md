...
description: Use when HQ operations need structured telemetry events written to .hq/telemetry/ for observability, eval input, or the weekly self-improvement loop. Covers task completions, skill invocations, policy decisions, hook triggers, and session boundaries.
---
...
Each event is a newline-delimited JSON object appended to `.hq/telemetry/YYYY-MM/events.jsonl`:
...
| `self_improvement.proposal` | improvement filed to .hq/improvements/ |
...
3. Determine the log path: `.hq/telemetry/YYYY-MM/events.jsonl` (create the month directory if absent).
...
- `.hq/telemetry/` is private runtime state: never add it to git.
